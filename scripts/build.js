...
async function generateBundledModule(inputFile, outputFile, format, production) {
  ...
  await bundle.write({
    file: outputFile,
    format,
    banner: '/** Author Rain120: https://github.com/Rain120 */',
    exports: 'named',
...
async function build() {
  ...
  await Promise.all([
    generateBundledModule(es5Build, path.join('lib', `${pkg.name}.js`), 'cjs', false),
    generateBundledModule(es5Build, path.join('lib', `${pkg.name}.min.js`), 'cjs', true),

    generateBundledModule(es5Build, path.join('lib', `${pkg.name}.module.js`), 'es', false),

    generateBundledModule(es6Build, path.join('lib', `${pkg.name}.es6.js`), 'es', false),

    generateBundledModule(es5Build, path.join('lib', `${pkg.name}.umd.js`), 'umd', false),
    generateBundledModule(es5Build, path.join('lib', `${pkg.name}.umd.min.js`), 'umd', true),
  ]);
